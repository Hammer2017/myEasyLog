...
std::ostream& operator<<(std::ostream& ss, const StackTrace::StackTraceEntry& si) {
  ...
  if (!si.m_demangled.empty()) {
    targetAddr = si.m_demangled.substr(si.m_demangled.find('[') + 1, si.m_demangled.find(']') - si.m_demangled.find('[') - 1);
  }
  ...
  //获取当前运行程序所在目录
  auto getCurrentExecDir = []() -> std::string {
  ...
  // 构造 addr2line 命令
  std::string command = "addr2line -e " + getCurrentExecDir() + " " + targetAddr + " -fCi";
  // 执行命令并获取输出
  ...
  ss << "[" << si.m_index << "] " << si.m_location << (si.m_hex.empty() ? "" : "+") << si.m_hex << " " << si.m_addr <<
     (si.m_demangled.empty() ? "" : ":") << si.m_demangled << (outPut.empty() ? "" : " " + outPut);
  return ss;
}
